...
// 09/04/2025 - Begins to Boot Linux but stalls
...
// Worked before now not Booting at all
class RocketPrefetchTemplate extends Config(
  new barf.WithHellaCachePrefetcher(Seq(0), barf.SingleNextLinePrefetcherParams()) ++
  new freechips.rocketchip.subsystem.WithNonblockingL1(1) ++
  new barf.WithTLDCachePrefetcher(barf.SingleStridedPrefetcherParams()) ++
  new chipyard.config.WithTilePrefetchers ++
  new freechips.rocketchip.subsystem.WithInclusiveCache(capacityKB=4) ++
  new freechips.rocketchip.subsystem.WithNBigCores(1) ++
  new chipyard.config.AbstractConfig)

class RocketPrefetchSimple extends Config(
  // new barf.WithTLDCachePrefetcher(barf.NLPrefetcherParams()) ++
  new chipyard.config.WithTilePrefetchers ++                                           
  new freechips.rocketchip.subsystem.WithNBigCores(1) ++ 
  new chipyard.config.AbstractConfig)

class RocketNLSimple extends Config(
  new barf.WithTLDCachePrefetcher(barf.NLPrefetcherParams()) ++
  new chipyard.config.WithTilePrefetchers ++                                           
  new freechips.rocketchip.subsystem.WithNBigCores(1) ++ 
  new chipyard.config.AbstractConfig)

class RocketNLPrefetch extends Config(
  new barf.WithTLDCachePrefetcher(barf.NLPrefetcherParams()) ++
  new chipyard.config.WithTilePrefetchers ++
  new freechips.rocketchip.subsystem.WithNonblockingL1(1) ++
...
// Markov Prefetcher to test
class RocketMarkovPrefetch extends Config(
  new barf.WithTLDCachePrefetcher(barf.StagedMarkovCorrelatingPrefetcherParams()) ++
  new chipyard.config.WithTilePrefetchers ++
  new freechips.rocketchip.subsystem.WithNonblockingL1(1) ++
  new freechips.rocketchip.subsystem.WithInclusiveCache(capacityKB=128) ++ 
  new freechips.rocketchip.subsystem.WithNBigCores(1) ++
